...
void readLFSsensors()
{
  ...
  farLeftSensor = analogRead(farLeftSensorPin); //in og code, black is low value, <100. White high value > 100 
                                                //with my sensors, black is high value > 900 and white is low <860.... weird
  if     ((LFSensor[0]== 0 )&&(LFSensor[1]== 0 )&&(LFSensor[2]== 0 )&&(LFSensor[3]== 0 )&&(LFSensor[4]== 0 ))  {mode = CONT_LINE; error = 0;}
  else if((LFSensor[0]== 1 )&&                                                   (farRightSensor > THRESHOLD)) {mode = RIGHT_TURN; error = 7;}
  else if ((farLeftSensor >  THRESHOLD)                                                  &&(LFSensor[4]== 1 )) {mode = LEFT_TURN; error = 8;}
  else if((LFSensor[0]== 1 )&&(LFSensor[1]== 1 )&&(LFSensor[2]== 1 )&&(LFSensor[3]== 1 )&&(LFSensor[4]== 1 ))  {mode = NO_LINE; error = 0;}
  else if((LFSensor[0]== 1 )&&(LFSensor[1]== 1 )&&(LFSensor[2]== 1 )&&(LFSensor[3]== 1 )&&(LFSensor[4]== 0 ))  {mode = FOLLOWING_LINE; error = 4;}
